...
def objective(trial):    
    ...
    momentum = trial.suggest_categorical('momentum', [0.05, 0.1, 0.3, 0.5, 0.7, 0.95, 1])
    weight_decay = trial.suggest_categorical('weight_decay', [0.0005, 0.005, 0.01, 0.1])
    num_epochs = trial.suggest_categorical('number epochs', [50, 100, 150, 200, 250])
    ...
    average_train_loss = mean(training_loss)
    ...
    average_val_loss = mean(validation_loss)

    return average_val_loss, average_train_loss 
...
def optimize():
    ...
    print('Writing Parameters to csv')
    with open(file_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Trial', 'Parameter', 'Value', 'Average Train Loss', 'Average Validation Loss'])      
        for trial in study.trials:
            parameters = trial.params
            
            for param_name, param_value in parameters.items():
                writer.writerow([trial.number, param_name, param_value, trial.values[0], trial.values[1]])

    print('Best trials:')
    best_trials = study.best_trials
    for trial in best_trials:
        print(f'Trial number: {trial.number}')
        print(f'Training loss: {trial.values[0]}')
        print(f'Validation loss: {trial.values[1]}')
        print()    
